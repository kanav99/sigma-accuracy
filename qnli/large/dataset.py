...
def main():
    ...
    # load dataset
    dataset = load_dataset("SetFit/qnli", split="validation")
    # sd = torch.load("model_mrpc.pth", map_location=torch.device('cpu'))
    model = AutoModelForSequenceClassification.from_pretrained("yoshitomo-matsubara/bert-large-uncased-qnli")
    sd = model.state_dict()
    ...
    for text_a, text_b, label in tqdm(zip(dataset["text1"], dataset["text2"], dataset["label"])):
        ...
        # print(x.shape)
        # set_file('../datasets/qnli_large_yoshitomo/' + str(i) + '.dat')
        g.write(str(label) + '\n')
